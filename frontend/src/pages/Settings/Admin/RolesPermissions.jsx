  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="border-b border-border/50 pb-2">
        <h3 className="text-medium font-semibold text-text">Roles & Permissions</h3>
        <p className="text-small text-text-muted">Platform access control matrix</p>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {[
              ...
              <tr key={row.name} className={`border-b border-border/30 hover:bg-border/10 transition-colors ${i % 2 === 0 ? '' : 'bg-border/5'}`}>
                <td className="py-3 px-4 text-small text-text font-medium">{row.name}</td>
                <td className="py-3 px-4 text-center">
                  {row.admin ? <Check className="h-4 w-4 text-status-info mx-auto" /> : <X className="h-4 w-4 text-text-muted mx-auto opacity-50" />}
                </td>
                <td className="py-3 px-4 text-center">
                  {row.user ? <Check className="h-4 w-4 text-status-info mx-auto" /> : <X className="h-4 w-4 text-text-muted mx-auto opacity-50" />}
                </td>
                <td className="py-3 px-4 text-center">
                  {row.viewer ? <Check className="h-4 w-4 text-status-info mx-auto" /> : <X className="h-4 w-4 text-text-muted mx-auto opacity-50" />}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="bg-primary/5 border border-primary/20 p-4 rounded flex gap-3">
        ...
        <div className="flex flex-col gap-1 text-small text-text">
