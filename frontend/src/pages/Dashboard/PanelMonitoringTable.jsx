...
const PanelMonitoringTable = ({ panels }) => {
  if (!panels || panels.length === 0) {
    return (
      <div className="p-8 text-center text-text-muted bg-surface border border-border rounded">
  ...
  return (
    <div className="border border-border rounded overflow-hidden bg-surface shadow-sm">
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-border/30 text-text-muted text-small uppercase tracking-wider">
        ...
        <tbody>
          {panels.map((panel, i) => {
            ...
            return (
              <tr key={panel.id} className={`border-b border-border/50 hover:bg-border/10 ${i % 2 === 0 ? '' : 'bg-border/5'}`}>
                <td className="py-2 px-4 text-text font-medium text-small">{panel.name}</td>
                <td className="py-2 px-4 text-text text-small text-right font-mono">{actual}</td>
                <td className="py-2 px-4 text-text text-small text-right font-mono">{expected}</td>
                <td className="py-2 px-4 text-text text-small text-right font-mono">{perf}</td>
