...
class PVGISService:
    ...
    @staticmethod
    async def get_panel_daily_profile(panel: Panel, site: Site) -> dict:
        """
        Returns a dictionary mapping UTC 'HH:MM' to expected power in Watts for a single panel.
        Used for mapping historical SensorReading timestamps to their expected power.
        """
        endpoint = "https://re.jrc.ec.europa.eu/api/v5_2/DRcalc"
        site_tz = ZoneInfo(site.timezone or "UTC")
        local_time = datetime.now(site_tz)
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                for record in daily_profile:
                    ...
                    profile_map[time_str] = round(power_w, 2)
            except Exception as e:
                print(f"PVGIS DRcalc Daily Profile Error for panel {panel.id}: {str(e)}")
